...
const MyDashboard = () => {
  ...
  const { userEmail } = useParams(); // Correctly extract userEmail
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await axios.post(`${http_typ}://${host_name_3001}/api/tasks/${userEmail}`);
  ...
  useEffect(() => {
    const fetchModals = async () => {
      try {
        const response = await axios.post(`${http_typ}://${host_name_3001}/api/modals/${userEmail}`);
          ...
          {sortedDetails
            ...
            .map((tasks) => (
              <tr key={tasks.id}>
                ...
                <td>{tasks.email}</td>
                <td>{formatDate(tasks.date)}</td>
                <td>{tasks.description}</td>
                <td>{tasks.location}</td>
                <td>{tasks.district}</td>
              </tr>
            ))}
        </tbody>
      </table>
