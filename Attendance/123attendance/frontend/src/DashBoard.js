...
const DashBoard = () => {
        ...
        const fetchUserData = async () => {
            try {
                ...
                const [
                    ...
                    await axios.get(`${http_typ}://${host_name_3005}/fetchWorkingHours/${email}`),
                    await axios.get(`${http_typ}://${host_name_3005}/fetchPrivilegeLeave/${email}`),
                    await axios.get(`${http_typ}://${host_name_3005}/fetchSickLeave/${email}`),
                    await axios.get(`${http_typ}://${host_name_3005}/fetchPermissions/${email}`)
                ]);
    ...
    useEffect(() => {
        ...
        const remainingSickLeave = Math.max(12 - approvedSickLeaveCount, 0);
    ...
    <tbody>
        {privilegeLeave.map(leave => (
            <tr key={leave.id}>
                ...
                <td>{leave.status}</td>
                {leave.status !== 'Approved' && <td>{leave.cancellation_reason}</td>}
            </tr>
        ))}
    </tbody>
</table>
    ...
    <tbody>
        {permissions.map(permission => (
            <tr key={permission.id}>
                ...
                {permission.status !== 'Approved' && <td>{permission.cancellation_reason}</td>}
            </tr>
        ))}
    </tbody>
</table>

<div className='h4-table'>
